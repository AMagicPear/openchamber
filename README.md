...
> ## ⚠️ This branch (`pi-adapt`) is a fork-in-progress for Pi backend support.
>
> Upstream OpenChamber targets the [OpenCode](https://opencode.ai) CLI. This fork
> retains the upstream frontend and codebase layout while the agent-backend is
> being adapted to the [Pi Coding Agent](https://github.com/badlogic/pi-mono).
> See `docs/decisions/0004-fork-openchamber-for-pi-feasibility.md` in the sibling
> `pichamber` repo for the recorded plan.
>
> Track this work on the `pi-adapt` branch. The default `main` branch tracks
> upstream OpenChamber and is intentionally unchanged.
